...
public:
    ...
    void operator()(ADvector &fg, const ADvector &vars) {
        ...
        const int psi_weight =5;
        const int u_weight = 2;
        const int v_weight = 2;
...
vector<double> Mpc::solve(Eigen::Vector3d state, Eigen::MatrixXd desired_state) {
    ...
    solved.push_back(solution.x[u_start]);  // u: forward velocity
    solved.push_back(solution.x[v_start]);  // v: lateral velocity
    solved.push_back(solution.x[r_start]);  // r: angular velocity
    for (int i = 0; i < N; ++i) {
        solved.push_back(solution.x[x_start + i]);
        solved.push_back(solution.x[y_start + i]);
    }
    cout << "u:" << solution.x[u_start] << " " << "v:" << solution.x[v_start] << " " << "r:" << solution.x[r_start] << endl;
    return solved;
}