...
本文件整合三层模块，提供 ROS 接口。
MPC 作为纯跟踪控制器，精确跟踪参考轨迹。

其他架构请参考:
    - mppi_two_layer.py: A* + MPPI (轨迹优化+控制一体化)
    - ilqr_two_layer.py: A* + iLQR (轨迹优化+控制一体化)
...
class ThreeLayerController:
    ...
    def __init__(self):
        ...
        # 性能指标收集器
        self.metrics = MetricsCollector(algo_name="MPC")
        ...
        self.mpc_tracker = MPCTracker(dt=self.dt, N=self.N)
    ...
    def _print_banner(self):
        """打印启动信息"""
        rospy.loginfo("=" * 50)
        rospy.loginfo("Three-Layer Controller (A* + Traj + MPC)")
